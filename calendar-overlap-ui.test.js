...
test('manual sync removes only duplicate Google events with the same KeeleSepp schedule id',()=>{
  ...
  assert.match(functionsSource,/privateExtendedProperty: `keeleseppScheduleId=\$\{scheduleId\}`/);
  ...
  assert.match(functionsSource,/compactEvents\.filter\(event =>\s*!event\.recurringEventId/);
