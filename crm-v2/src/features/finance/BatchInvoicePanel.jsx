...
import { batchInvoicePayload, buildBatchInvoicePlan, currentBillingMonth } from './batchInvoices.js';
    ...
    <Card className="batch-invoice-panel">
      <div className="batch-invoice-panel__intro">
        ...
        <div>
          ...
          <h3>Loo arved mitmele õpilasele</h3>
          <p>Koosta valitud kuu arveldamata tundidest eraldi arve igale õpilasele.</p>
        </div>
      </div>
      <div className="batch-invoice-panel__controls">
        ...
        <Button disabled={!plan.ready.length} onClick={() => { setDue(defaultInvoiceDue()); setError(''); setOpen(true); }}>
          <FileStack size={17} /> Vaata ja loo arved
        </Button>
      </div>
      {plan.blocked.length ? <p className="batch-invoice-panel__warning"><TriangleAlert size={17} /> {plan.blocked.length} õpilase arvet ei saa luua, sest tunni hind või õpilase kirje puudub.</p> : null}
