...
export default function ParentsPage({ repository = parentsService, studentRepository = studentsService, invoiceRepository = invoicesService, teacherRepository = teachersService }) {
  ...
  const createStudent = async (event) => {
    ...
    try {
      ...
      await repository.createMissingStudent(creatingRow.parent, { ...childForm, teacher: teacher?.name || '' }, state.data.students, user);
      setCreatingId('');
      setChildForm(null);
      setSuccess(`${childForm.name} õpilase kaart loodi ja seoti lapsevanemaga.`);
      await state.reload();
    } catch (error) {
